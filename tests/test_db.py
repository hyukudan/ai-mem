...
class DatabaseTests(unittest.TestCase):
    ...
    def test_tag_counts_and_replace(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            added = db.add_tag("urgent", project="proj", tag_filters=["beta"])
            self.assertEqual(added, 2)
            obs_a_added = db.get_observation(obs_a.id)
            obs_b_added = db.get_observation(obs_b.id)
            self.assertIn("urgent", obs_a_added["tags"])
            self.assertIn("urgent", obs_b_added["tags"])
            ...
            self.assertIn("beta", obs_b_updated["tags"])
            self.assertIn("urgent", obs_b_updated["tags"])

            updated_all = db.replace_tag("beta", "gamma", project="proj")
            self.assertEqual(updated_all, 1)
            obs_b_updated = db.get_observation(obs_b.id)
            self.assertIn("gamma", obs_b_updated["tags"])
            self.assertIn("urgent", obs_b_updated["tags"])
